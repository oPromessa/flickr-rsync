...
class RxSync(object):
    ...
    def run(self):
        ...
        src_folders = Observable.from_(
            self._src.list_folders()) .map(
            lambda folder: {
                'src': folder,
                'dest': dest_folders.get(
                    folder.name.lower())}) .publish().auto_connect(2)

        # Split into copy and merge streams
        to_copy = src_folders.filter(lambda x: x['dest'] is None)
        to_merge = src_folders.filter(lambda x: x['dest'] is not None)
        if self._config.root_files:
            to_merge = to_merge.start_with(
                {'src': RootFolderInfo(), 'dest': RootFolderInfo()})
        to_merge = to_merge.buffer(
            lambda: src_folders.when_complete()).flat_map(
            lambda x: x)
        ...
        pending .subscribe(
            lambda x: self._copy_file(
                **x) if not x['exists'] else logger.debug(
                "{}...skipped, file exists".format(
                    x['path'])))
    ...
    def _expand_folder(self, src, dest):
        is_merging = dest is not None
        logger.debug(
            "{} {} [{}]".format(
                "merging" if is_merging else "copying",
                src.name if not src.is_root else 'root',
                current_thread().name))
        dest_files = [f.name.lower()
                      for f in self._dest.list_files(dest)] if is_merging else []
